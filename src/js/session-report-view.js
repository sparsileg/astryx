...
const SessionReportView = {
    ...
    _buildReportHtml(fs, context) {
        ...
        html += this._buildVerdictHtml(fs);                          // §1
    ...
    // Imaging telescope/sensor, inferred from Astryx's own equipment
    // records — not from either log, which only describe the guide train.
    // Only confident when exactly one of each is registered (matching a
    // single-rig setup); with more than one telescope or sensor on file,
    // there's no signal in either log to say which was actually used, so
    // this says so explicitly rather than guessing wrong.
    _buildImagingEquipmentHtml() {
        ...
        if (telNames.length !== 1 || senNames.length !== 1) {
            return `<p class="session-report-note-small" style="color:var(--text-secondary)">Imaging telescope/sensor: ${telNames.length} telescope(s), ${senNames.length} sensor(s) configured in Astryx — can't infer which was used for this session from the logs alone.</p>`;
        }
        ...
        return `<p class="session-report-note-small"><strong>${HtmlUtils.escapeHtml(telNames[0])}</strong> / <strong>${HtmlUtils.escapeHtml(senNames[0])}</strong>${pixelScale != null ? ` (${pixelScale.toFixed(2)}"/px)` : ''}</p>`;
    },
    ...
    _buildRecommendedSettingsHtml(fs, context) {
        ...
        html += this._buildImagingEquipmentHtml();

        const flagged = fs.subs.filter(s => s.tier === 'marginal' || s.tier === 'reject');
        if (flagged.length > 0) {
            html += `<p class="session-report-note-small">Frames worth a closer look for defects (marginal/reject tier): ${flagged.map(s => s.imageNo).join(', ')}. Log analysis can't see image-level quality directly — see §Data Quality.</p>`;
        }
