...
const TUTORIAL_SEQUENCE_PLANNER = {
    ...
    steps: [
        ...
        // --- Overhead Settings ---
        {
            ...
            body: 'Below the session settings is the overhead configuration — the time costs that reduce your net imaging time each night. These include autofocus runs, meridian flips, calibration for guiding, and dithering. The planner subtracts these from the available window when calculating how many exposures you can collect.',
        ...
        },
        {
            id: 'calibration',
            type: 'callout',
            title: 'Guide Calibration and Frames per Dither',
            body: '<strong>Guide Calibration</strong> — The duration in minutes for guide calibration. Guide Calibration is assumed to run after moving to a new target (including the first) and after each meridian flip.<br><br><strong>Frames per Dither</strong> — How many exposures to take between dither events. Set to 0 to disable dithering entirely. The sub gap and dither duration used for timing calculations are learned automatically each time you analyze a session log.',
